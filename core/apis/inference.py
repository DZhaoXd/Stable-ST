...
@torch.no_grad()
def nfd_measure(feature_extractor, query_imgs, neigh_imgs):
    """
    Args:
        feature_extractor: nn.Module, 输出最后一层特征，形状 [B, d, h, w]
        query_imgs:  Tensor [B, 3, H, W]
        neigh_imgs:  Tensor [B, 3, H, W]，与 query_imgs 一一对应的稳定邻居
    """
    device = next(feature_extractor.parameters()).device
    feature_extractor.eval()

    query_imgs = query_imgs.to(device, non_blocking=True)
    neigh_imgs = neigh_imgs.to(device, non_blocking=True)

    Fq = feature_extractor(query_imgs)[1].permute(0, 2, 3, 1).contiguous()  # [B, h, w, d]
    Fn = feature_extractor(neigh_imgs)[1].permute(0, 2, 3, 1).contiguous()  # [B, h, w, d]

    S = (Fq * Fn).sum(dim=-1, keepdim=False)     # [B, h, w]
    B, h, w = S.shape
    P = F.softmax(S.view(B, -1), dim=1)          # [B, h*w]
    entropy = -(P.clamp_min(1e-12) * P.clamp_min(1e-12).log()).mean(dim=1)  # [B]
    nfd = 1.0 - entropy
    return nfd.detach().cpu().tolist()
...
def evel_stu(cfg, feature_extractor, classifier, stu_eval_list):
    ...
    if cfg.DTU.PROXY_METRIC == 'NFD':
        all_scores = []
        for item in stu_eval_list:
            # item: [query_imgs_cpu, neigh_imgs_cpu]
            query_imgs = item[0]
            neigh_imgs = item[1]
            scores = nfd_measure(feature_extractor, query_imgs, neigh_imgs)  # List[float]
