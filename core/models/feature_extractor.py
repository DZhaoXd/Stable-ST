...
from core.models.modules.backbones.mit import get_mit
from core.models import vgg, resnet
...
class resnet_feature_extractor(nn.Module):
    ...
    def forward(self, x):
        feas = self.backbone(x)
        return feas['aux'], feas['out']


class mit_feature_extractor(nn.Module):
    # backbone为字典型参数
    def __init__(self,backbone):
        super(mit_feature_extractor, self).__init__()
