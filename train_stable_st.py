...
def train(cfg, local_rank, distributed):
    logger = logging.getLogger("StableST.trainer")
        ...
        for i, (tgt_input, y, names, tgt_trans_param, tgt_img_full, mix_label) in enumerate(tgt_train_loader):
            ...
            if cfg.DTU.DYNAMIC:
                with torch.no_grad():
                    ...
                    elif cfg.DTU.PROXY_METRIC == 'NFD':
                        # 1) full-image softmax 
                        probs_full = output  # [B, C, H, W]
                        with torch.no_grad():
                            ent_map = -(probs_full.clamp_min(1e-12) * probs_full.clamp_min(1e-12).log()).sum(dim=1)  # [B, H, W]
                            ent_score = ent_map.mean(dim=(1, 2))  # [B]
                    
                        # 2) Q_style
                        def low_freq_amp(x, keep_ratio=0.125):
                            # x: [B, 3, H, W], 先转灰度以简化
                            x_gray = x.mean(dim=1, keepdim=True)
                            Xf = torch.fft.rfft2(x_gray, norm='ortho')               # [B, 1, H, W//2+1]
                            amp = Xf.abs()                                          
                            H, W2 = amp.shape[-2], amp.shape[-1]
                            h_keep = max(1, int(H * keep_ratio))
                            w_keep = max(1, int(W2 * keep_ratio))
                            return amp[:, :, :h_keep, :w_keep].flatten(1)            # [B, M]
                    
                        with torch.no_grad():
                            style_vec = low_freq_amp(tgt_img_full.clone().detach())  # [B, M]
                            style_vec = F.normalize(style_vec, dim=1)
                     
                        B = tgt_img_full.size(0)
                        pairs = []          
                        pair_indices = []   
                        ent_rank = torch.argsort(ent_score)  
                        stable_pool = ent_rank[: max(1, B // 2)] 
                    
                        for i in range(B):
                            sim = (style_vec[i:i+1] @ style_vec[stable_pool].T).squeeze(0)  # [|stable_pool|]
                            mask_self = (stable_pool == i)
                            if mask_self.any():
                                sim = sim.masked_fill(mask_self.nonzero(as_tuple=True)[0], -1.0)
                            j = stable_pool[torch.argmax(sim)]
                            pairs.append((tgt_img_full[i].cpu(), tgt_img_full[j].cpu()))
                            pair_indices.append((i, int(j)))
                    
                        with torch.no_grad():
                            Fq = feature_extractor(tgt_img_full)[1]     
                            Fq = Fq.permute(0, 2, 3, 1).contiguous()    # [B, h, w, d]
                    
                            # 为邻居也提特征
                            neigh_imgs = torch.stack([tgt_img_full[j] for (_, j) in pair_indices], dim=0).to(Fq.device)  # [B, 3, H, W]
                            Fn = feature_extractor(neigh_imgs)[1].permute(0, 2, 3, 1).contiguous()                       # [B, h, w, d]
                    
                            # S = <Fq, Fn>  -> [B, h, w]
                            S = (Fq * Fn).sum(dim=-1, keepdim=True)  # [B, h, w, 1]
                            S_flat = S.view(S.size(0), -1)           # [B, h*w]
                            P = F.softmax(S_flat, dim=1)             # [B, h*w]
                            entropy = -(P.clamp_min(1e-12) * P.clamp_min(1e-12).log()).mean(dim=1)  # [B], 已经是 1/(wh)*sum E[·]
                            nfd_val = 1.0 - entropy  # [B]
                    
                        stu_score_buffer.extend(nfd_val.tolist())
                        stu_eval_list.append([tgt_img_full.clone().detach().cpu(),
                                              torch.stack([tgt_img_full[j] for (_, j) in pair_indices]).cpu()])
                    else:
                        print('no support')
                        return
            ###########
...
def main():
    ...
    logger = setup_logger("StableST", output_dir, args.local_rank)
